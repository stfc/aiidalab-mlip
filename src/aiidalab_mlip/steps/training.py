...
class TrainingWizardStep(ipw.VBox, awb.WizardAppWidgetStep):
    ...
    def __init__(self, model, **kwargs) -> None:
        ...
        self.mode_selector = ipw.ToggleButtons(
            options=[("Use Pre-trained Model", "pretrained"), ("Train Custom Model", "train")],
            value="pretrained",
            description="Mode:",
            button_style="info",
        )
        ...
        self.train_button = ipw.Button(
            description="Configure Training", button_style="warning", disabled=False
        )
        self.train_button.on_click(self._on_train_click)

        self.continue_button = ipw.Button(
            description="Continue with Pre-trained Model →", button_style="success", disabled=False
        )
        ...
        super().__init__(
            children=[self.title, self.mode_selector, self.mode_area, self.status, self.output],
            **kwargs,
        )
